...
def main() -> None:
    ...
    parser.add_argument("--head", choices=["fpn", "zipdepth"], default="fpn",
                        help="depth head: repo FPN head or ZipDepth (ECCV'26)")
    parser.add_argument("--zipdepth-variant", default="base")
    parser.add_argument("--zipdepth-ckpt", default=None, help="optional zipdepth_base.pth to init the head")
    ...
    train_ds = M3EDDepthDataset(**common, split="train")
    val_ds = M3EDDepthDataset(**common, split="val")
    ...
    if args.baseline:
        ...
        if args.head == "zipdepth":
            ...
            model.depth_head = ZipDepthHead(
                ...
                variant=args.zipdepth_variant, pretrained_ckpt=args.zipdepth_ckpt,
            ).to(device)
    ...
    if args.head == "zipdepth" and not args.baseline and args.tag is None:
        tag += "_zipdepth"
